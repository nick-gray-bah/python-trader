...
class Stock:
    ...
    def fetch_data(self, start, end):
        # Fetch historical data from yahoo finance for period start-end
        bars = self.yf.download(self.ticker, start=start, end=end)
        
        if bars.empty:
            raise ValueError(f"No data returned for {self.ticker} between {start} and {end}")
    ...
    def RSI(self, prices, n=14):
        rsi = RSIIndicator(prices, window=n).rsi()
        return rsi
    ...
    def graph(self, movingAverageArr=[]):
        try:
            ...
            ax0.plot(self.dates[-SP:], self.rsi[-SP:], rsiCol, linewidth=1.5)
            ...
            ax2.plot(self.dates[-SP:], macd[-SP:], color='#4ee6fd', lw=2)
            ax2.plot(self.dates[-SP:], ema9[-SP:], color='#e1edf9', lw=1)
